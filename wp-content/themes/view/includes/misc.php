...
function get_mustache_templates() {
	return array_reduce(
		glob( PATH . 'views/mustache/*.mustache' ),
		function ( $result, $file ) {
			$result[ basename( $file, '.mustache' ) ] = [
				'path'     => URI . 'views/frontend/mustache/' . basename( $file ),
				'template' => get_file_as_string( $file ),
			];
			return $result;
		}
	);
